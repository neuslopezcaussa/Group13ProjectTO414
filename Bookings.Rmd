...
bookings.dummy <- dummy_cols(bookings, remove_first_dummy=TRUE)
...
bookings_train <- subset(bookings, split == TRUE)
bookings_test <- subset(bookings, split == FALSE)
...
plot(as.factor(bookings$is_canceled), main="How often customers cancel their bookings?", ylab="Number of cancelations",  xlab= "Is cancled", ylim = c(0,90000), col=c("green", "red"), legend.text = "0 = No, 1 = Yes", width=20)
```
...
model1 <- glm(is_canceled ~ adults + children.s + babies.s + distribution_channel + is_repeated_guest + previous_cancellations + previous_bookings_not_canceled + customer_type, family = "binomial", data = bookings)
summary(model1)

## Revise the variables!!!
### Secondly, we will analyze the impact of the variables related to the reservation:
#model1.2 <- glm(is_canceled ~ lead_time + arrival_date_week_number + stays_in_weekend_nights + stays_in_week_nights + meal  + reserved_room_type + assigned_room_type + booking_changes + deposit_type + agent + company + days_in_waiting_list + required_car_parking_spaces + total_of_special_requests, family = binomial, data = bookings)
summary(model2)
```
...
library(class)
## Note: we will use k= 345 as it is its square root.
model2<- knn(train = bookings_train[ ,-2], test = bookings_test[ ,-2], cl = bookings_train_labels, k = 34)

# 2. Model Evaluation:
library(caret)
confusionMatrix(as.factor(model2), as.factor(pokemon_test_labels))
```
